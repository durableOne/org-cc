    ...
    (if logbook
        (save-match-data
          ...
          (let ((clock (match-string 1 logbook))) 
            (if clock
                (progn
                  ;; Match either [timestamp] (in case clock is still active)
                  ;; or -[timestamp] to retrieve the last time you clocked out.
                  (string-match "\\(-\\|^\\)\\[\\([^\]]+\\)\\]" clock)
                  (org-time-stamp-to-now (match-string 2 clock)))
              nil)))
      nil)))
